...
export default function Bracket({ series, picks }: BracketProps) {
  ...
  const handleSaved = useCallback((seriesId: string, pickedTeamId: string, gamesPrediction?: number) => {
    setLocalPicks((prev) => {
      ...
      if (existing >= 0) { const updated = [...prev]; updated[existing] = newPick; return updated; }
      return [...prev, newPick];
    });
  ...
  function renderColumn(groupSeries: Series[]) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", justifyContent: "space-around" }}>
        {groupSeries.map((s) => (
          <SeriesCard key={s.id} series={s} pick={pickMap.get(s.id)} oddsMap={oddsMap} showOdds={showOdds} onPickClick={handlePickClick} />
        ))}
        {groupSeries.length === 0 && (
          <div style={{ minWidth: 130, maxWidth: 160, height: 80, background: "var(--bg-card)", border: "1px dashed var(--border)", borderRadius: "var(--radius)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-dim)", fontSize: "0.78rem" }}>TBD</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.25rem", flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: "0.8rem", color: "var(--text-muted)", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "0.3rem 0.7rem" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.25rem" }}>
          <span style={{ fontSize: "0.78rem", color: "var(--text-muted)", fontWeight: 600 }}>{picksMade} / 15 picks</span>
          <div style={{ width: 120, height: 5, background: "var(--border)", borderRadius: 999, overflow: "hidden" }}>
            <div style={{ width: `${progressPct.toFixed(1)}%`, height: "100%", background: "var(--accent)", borderRadius: 999, transition: "width 0.3s ease" }} />
          </div>
        </div>
      </div>

      <div className="no-scrollbar" style={{ overflowX: "auto", paddingBottom: "1rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(155px, 1fr))", gap: "1rem", minWidth: 1100 }}>
          {["R1 · West", "Conf Semis", "Conf Finals", "NBA Finals", "Conf Finals", "Conf Semis", "R1 · East"].map((label, i) => (
            <div key={i} style={{ textAlign: "center", fontSize: "0.72rem", fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.06em", textTransform: "uppercase", paddingBottom: "0.5rem", borderBottom: "1px solid var(--border)" }}>{label}</div>
          ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: "0.5rem" }}>
            ...
            {finalsGroup?.finals ? (
              ...
              <div style={{ minWidth: 130, maxWidth: 160, height: 80, background: "var(--bg-card)", border: "1px dashed var(--accent)", borderRadius: "var(--radius)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--accent)", fontSize: "0.78rem", fontWeight: 600 }}>NBA Finals</div>
            )}
          </div>
          <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>{renderColumn(series.filter((s) => s.round === 3 && s.conference === "East"))}</div>
      ...
      {activeSeries && (
        <PickModal series={activeSeries} currentPickTeamId={pickMap.get(activeSeries.id)?.picked_team_id} currentGames={pickMap.get(activeSeries.id)?.games_prediction ?? undefined} onClose={() => setActiveSeries(null)} onSaved={handleSaved} />
      )}
