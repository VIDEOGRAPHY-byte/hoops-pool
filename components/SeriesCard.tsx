    ...
    return (
      <div
        style={{
          ...
          borderRadius: 6,
          background: isPicked ? "var(--accent-glow)" : isWinner ? "rgba(34,197,94,0.08)" : "transparent",
          opacity: isComplete && !isWinner ? 0.4 : 1,
          outline: isPicked && showOdds ? "1px dashed var(--accent)" : "none",
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
          {isPicked && <span style={{ fontSize: "0.65rem", color: "var(--accent)" }}>\u2713</span>}
          {isWinner && <span style={{ fontSize: "0.65rem" }}>\u{1F3C6}</span>}
          <span style={{ fontSize: "0.75rem", color: "var(--text-muted)", minWidth: 16, fontWeight: 600 }}>{team.seed}</span>
          <span style={{ fontSize: "0.85rem", fontWeight: isPicked || isWinner ? 700 : 500, color: isPicked ? "var(--accent)" : isWinner ? "var(--success, #22c55e)" : "var(--text)" }}>
            {team.abbreviation}
          </span>
        </div>
        {winProb !== undefined && (
          <span style={{ fontSize: "0.7rem", color: "var(--text-dim)" }}>{(winProb * 100).toFixed(0)}%</span>
        )}
        {!winProb && odds && (
          <span style={{ fontSize: "0.7rem", color: "var(--text-dim)" }}>
            {odds.championship_odds > 0 ? `+${odds.championship_odds}` : odds.championship_odds}
          </span>
        )}
      </div>
    );
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${hasPick ? "var(--accent)" : "var(--border)"}`,
        borderRadius: "var(--radius)",
        padding: "0.6rem 0.5rem",
        cursor: canPick ? "pointer" : locked ? "not-allowed" : "default",
        transition: "border-color 0.15s, box-shadow 0.15s",
        minWidth: 130,
        maxWidth: 160,
        position: "relative",
        boxShadow: hasPick ? "0 0 0 1px var(--accent)" : "none",
      }}
    ...
    >
      ...
      {team_a ? (
        <TeamRow team={team_a} isWinner={winner?.id === team_a.id} isPicked={pickedId === team_a.id} />
      ) : (
        <div style={{ color: "var(--text-dim)", fontSize: "0.8rem", padding: "0.3rem 0.5rem" }}>Awaiting winner</div>
      )}

      <div style={{ height: 1, background: "var(--border)", margin: "0.2rem 0" }} />

      {team_b ? (
        <TeamRow team={team_b} isWinner={winner?.id === team_b.id} isPicked={pickedId === team_b.id} />
      ) : (
        <div style={{ color: "var(--text-dim)", fontSize: "0.8rem", padding: "0.3rem 0.5rem" }}>Awaiting winner</div>
      )}
      ...
      {hasPick && (
        ...
        >
          ...
          {pick?.games_prediction && (
            <span style={{ fontSize: "0.65rem", color: "var(--text-dim)" }}>in {pick.games_prediction}</span>
          )}
        </div>
      )}
    </div>
